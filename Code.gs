...
function fetchPatientsWithoutUpcomingAppointmentsReport(practitionerId, startDate, endDate, businessId) {
  ...
  var reportBaseUrl = config.reportBaseUrl || '';
  reportBaseUrl = reportBaseUrl.replace(/\/reports\/patients\/without_upcoming_appointments(?:\.csv)?$/, '');
  var url = buildUrl(reportBaseUrl + endpoint, params);
  var response;
  try {
